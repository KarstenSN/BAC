...
\subsection{Målerkredsløb ver.01}

Der er designet et kredsløb der konverterer faseændringen om til en DC-spænding som herefter samples af mikroprocessorens ADC. Dette kredsløb ses på Figur \ref{fig:jf2}. Kredsløbet fungerer ved at der påtrykkes en 100kHz 3.3V firkant-spænding over jordspyddet som loades af R25. Dette efterfølges af en spændingsfølger således R25 kan vælges uden forbehold for det efterfølgende kredsløb. Firkant-spændingen filtreres herefter af et båndpass-filter således at der ligger en sinus på udgangen af U12A. Denne sinus er $180^\circ$ faseforskudt da U12A er koblet som en inverterende forstærkter, derfor sættes der en komparator på udgangen af U12A for at returnere fasen til $0^\circ$. Til orientering, vil der her blive introduceret et propagation delay som vil bidrage en smule til faseforskydelsen, der forventes i omegnen af $80ns$ \cite{lib:comp_MCP6561_datasheet} der kan derfor vælges at se bort fra dette i denne situation. Ved at køre frekvensen fra mikroprocessoren samt udgangen af U8A igennem en XOR-gate (U10A). U10A omdanner faseforskellen af de 2 signaler til et PWM signal, dette variere med faseforskellen således at der fås en lav duty-cycle ved lille faseforskel og $50\%$ duty cycle ved maksimal faseforskydning, dette signal vil oplade en DC-spænding på C26 som vil afhænge af faseforskydelsen. denne spænding forstærkes til slut op af U12B for at give en højere dynamisk range til ADC'en.
...
\subsection{Målerkredsløb beregninger}
...
\subsection{Modultest - Målerkredsløb ver.01}
Da kredsløbet, til test, blev monteret på et PCB (sensor\_3.brd som bruger type 4 jordspyd) stod det hurtigt klart at D5 var ødelæggende for signalet, da denne klipper den negative del af pulsen under -0.6V. U8B var fejlagtigt en komparator og duede derfor ikke som spændingsfølger, dette er tydeligt verificeret af Figur \ref{fig:scopeU8B}. Endvidere var C35 monteret forkert da denne bør sidde parallelt med R28 som båndbredde-begrænsning. Det viste sig ydermere at faseforsinkelsen igennem kredsløbet var så stor, at indputtet til U1A ben A skulle have sin egen separate puls hvor forsinkelsen kan styres via software. 
...
\subsection{Målerkredsløb ver.02}
...
Her er indført en MOSFET-transistor, Q11 således at mikroprocessoren kan tænde og slukke for et separat måle-groundplan. Dette er gjort af strømbesparende årsager. Hermed kan der lukkes ned for hele målerkredsløbet når sensoren lægges i sleep-mode. I mikroprocessoren er der en automatisk kalibreringsfunktion som køres ved tryk på Connect-knappen. \todo[inline]{Ændring af alle henvisninger fra RESET-knap til Connect}
...
\subsubsection{U12A/B}
Der er valgt at erstatte den hidtil benyttede TL972 Operationsfortærker med modellen MCP6102t fra Microchip \cite{lib:opamp_MCP6102t_datasheet}. Denne nye opamp er valgt på baggrund af dens mere passende båndbredde på $1Mhz$. Dertil har den lavere Quissent Current på $85 \mu A$.Derudover har den en slew rate på $0.6V/ \mu s$ der opfylder kravet til systemet som ses i ligning \ref{equ:slewrate}. Derudover opfylder chippen temperaturkravene.  
...
\subsection{Modultest - Målerkredsløb ver.02}
...
\begin{figure}[H]
	...
	\caption{Faseforskydelse af indgang og udgang i båndpass-filteret ved 20pF}
	\label{fig:j15}
\end{figure} 
...
\begin{figure}[H]
	...
	\caption{Faseforskydelse af indgang og udgang i båndpass-filteret ved 120pF}
	\label{fig:j16}
\end{figure} 
...
\begin{figure}[H]
	...
	\caption{Fasemargin vs forsyningsspænding}
	\label{fig:j17}
\end{figure} 
