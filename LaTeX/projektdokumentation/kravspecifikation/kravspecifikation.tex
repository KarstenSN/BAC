...
I denne sektion opstilles de funktionelle krav til systemet. Disse krav skal opfyldes for at systemet betragtes som fuldt funktionelt. I tabel \ref{tbl:funk_krav} på side \pageref{tbl:funk_krav} listes de funktionelle krav med unik ID så der i systemet er fuld traceability. kolonnen \textit{Prioritet} henviser til MoSCoW-prioriteringen der er beskrevet i afsnit \ref{sec:moscow}. Kravene er typebenævnt i forhold til følgende konvention.
...
CM\_03	& Sensor og kontrolboks skal parres automatisk 														& \textit{Must}	 & 	\\ \hline
CM\_04	& Systemets parring skal nulstilles ved udtagelse af batteri i sensor								& \textit{Must}	 & 	\\ \hline
...
UX\_05	& Brugeren skal kunne indstille et ønsket vandingsinterval											& \textit{Must}	 & 	\\ \hline
UX\_06	& Brugeren skal kunne indstille en ønsket jordfugtighed på kontrolboksen.							& \textit{Must}	 & 	\\ \hline
UX\_07	& Brugeren skal kunne indstille en tidsbaseret vandingssekvens										& \textit{Must}  & 	\\ \hline
UX\_08	& Brugeren skal kunne vælge automatisk vandingstidsrum, eks. morgen, aften, begge eller uafhængig 	& \textit{Must}	 & 	\\ \hline		
	\end{tabularx}
...
I de følgende tabeller herunder ses de samme funktionelle krav som i tabel \ref{tbl:funk_krav}, dog er de her arrangeret efter MoSCoW-modellen. Under \textit{Must} placeres de krav som skal være opfyldt for at systemet kan siges at være minimum funktionsdygtigt. \textit{Should} indeholder de krav der bør være implementeret for at systemet kan kaldes fuldt funktionsdygtigt, disse krav implementeres hvis tid og ressourcer tillader det. \textit{Could} indeholder de krav som systemet ville ha kunne opfylde hvis tid og ressourcer ikke er en begrænsning. \textit{Would} er fremtidige krav til systemets efterfølgende iterationer, disse er derfor ikke implementeret i denne prototype. 

\textbf{Must}
\begin{table}[H]
	\begin{tabularx}{\textwidth}{ m{1.5 cm}  Z  L{1.9 cm} m{2.9 cm} }
		MC\_01 	& Systemet skal bestå af en sensor samt en kontrolboks  												\\
		MC\_05	& Sensoren skal måle temperatur, jordfugtighed samt lysintensitet										\\
		MC\_06	& Kontrolboks og sensor skal kunne passe i plasthus (B:83mmm H:59mm, D:34mm)							\\		
		MC\_07	& Display'et må have maksimal mål på (B:53mmm H:22mm)													\\
		MC\_08	& Display'et skal være baggrundsbelyst																	\\
		MC\_09	& Tasteturet må have maksimal mål på (B:70mmm H:23mm)													\\
		MC\_10	& Motorventilen skal kunne tilsluttes 1/2" vandrør														\\
		...
		EL\_03	& Kontrolboksen skal kunne forsynes med en 3. parts 5V AC/DC adaptor									\\
		CM\_01	& Kontrolboksen skal kunne udveksle data med sensoren via en trådløs forbindelse 						\\
		CM\_03	& Sensor og kontrolboks skal parres automatisk 															\\
		CM\_04	& Systemets parring skal nulstilles ved udtagelse af batteri i sensor									\\
		...
		UX\_05	& Brugeren skal kunne indstille et ønsket vandingsinterval												\\
		UX\_06	& Brugeren skal kunne indstille en ønsket jordfugtighed på kontrolboksen								\\
		UX\_07	& Brugeren skal kunne indstille en tidsbaseret vandingssekvens											\\
		UX\_08	& Brugeren skal kunne vælge automatisk vandingstidsrum, eks. morgen, aften, begge eller uafhængig		\\
	\end{tabularx}
	\caption{Must krav}
	\label{tbl:must_krav}
\end{table}
...
\begin{table}[H]
	\begin{tabularx}{\textwidth}{ m{1.5 cm}  Z  L{1.9 cm} m{2.9 cm} }
		CM\_02	& Kommunikation fra sensor til kontrolboks skal virke ved minimum 30m				\\
		UX\_02	& Kontrolboksen skal kunne måle om slangen er sprunget fra, og give fejlmelding 	\\
		UX\_09	& Brugeren skal kunne indstille en tidsbaseret vandingssekvens						\\
	\end{tabularx}
	\caption{Should krav}
...
\begin{table}[H]
	\begin{tabularx}{\textwidth}{ m{1.5 cm}  Z  L{1.9 cm} m{2.9 cm} }
		...
		MC\_04	& Systemet skal kunne tilsluttes flere sensorer					\\
	\end{tabularx}
	\caption{Would krav}
...
Funktionalitetskravene er der taget hånd om i afsnit \ref{sec:funktionelle_krav}. De ikke funktionelle krav er indekseret som det ses på de følgende tabeller, som med de funktionelle krav tildeles her unik krav ID. Kravene er typebenævnt i forhold til følgende konvention.
...
\begin{table}[H]
	\begin{tabularx}{\textwidth}{| m{1.5 cm} | Z | m{2.9 cm} |} \hline
		...
		US\_03		 & Displayet skal kunne vise jordfugtighed samt temperatur				 &	\\ \hline
...
\begin{table}[H]
	\begin{tabularx}{\textwidth}{| m{1.5 cm} | Z | m{2.9 cm} |} \hline
		\textbf{ID:} & \textbf{Krav:}														& \textbf{Kommentarer:} \\ \hline
		PF\_01		 & Batteritype (Alkaline AAA) 											& \\ \hline
		PF\_02		 & Sensorens batterilevetid (min 1 år)  								& \todo[inline]{test??}\\ \hline
		...
		PF\_05		 & Temperaturmåleren skal kunne måle fra ($ -20 - 70 ^{\circ}C $)								& \\ \hline
		PF\_06		 & Målepræcision (5\%)													& \todo[inline]{hvordan test vi?}\\ \hline
		PF\_07		 & Systemrespons fra brugerinteraktion til aktuator skal være 1<sek		& \todo[inline]{hvordan test vi?}\\ \hline
		PF\_08		 & Sensoren skal kunne måle temperatur og jordfugtighed med min. 1 decimals præcision	& \\ \hline
		PF\_09		 & Sensoren skal være funktionsdygtig i temperaturintervallet ($ -20 - 70^{\circ}C $) 		& \\ \hline
	\end{tabularx}
